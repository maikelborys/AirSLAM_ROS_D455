"""Visual odometry on EuRoC — XFeat + LighterGlue (the headline config).

Same shape as vo_euroc_xfeat.launch.py but points at the LighterGlue
config (matcher=3, kpts=512, KF-rate tuned).

ATE on EuRoC MH_03_medium: raw VO 0.207 m, post-refinement 0.070 m at
51.3 FPS — see output/benchmarks_xfeat.md "BEST" row.
"""
import os
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument('config_path',
            default_value=os.path.join(pkg_share, 'configs', 'visual_odometry', 'vo_euroc_xfeat_lighterglue.yaml')),
        ...
        DeclareLaunchArgument('saving_dir', default_value='/tmp/airslam_xfeat_lg'),
